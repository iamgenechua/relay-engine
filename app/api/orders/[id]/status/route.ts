...
export async function PATCH(
  ...
  if (!validTransitions.includes(newStatus)) {
    return NextResponse.json(
      {
        error: `ERR_INVALID_TRANSITION: Cannot transition from ${currentStatus} to ${newStatus}. Operation rejected.`,
      },
      { status: 422 }
    )
  }
  ...
  const { rows: updatedRows } = await sql`
    ...
    WHERE id = ${id}
    RETURNING id, customer_name, customer_email, status, total, created_at, updated_at
  `
